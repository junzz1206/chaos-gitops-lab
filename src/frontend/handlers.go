...
func (fe *frontendServer) homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	// currencies: 실패해도 fatal 아님
	currencies, err := fe.getCurrencies(r.Context())
	if err != nil {
		log.WithError(err).Warn("could not retrieve currencies")
		errMsg += "환율 정보를 불러오지 못했습니다. "
		// nil이어도 템플릿 range는 문제 없음
	}

	// products: 실패하면 빈 목록 + 배너로만 알림
	products, err := fe.getProducts(r.Context())
	if err != nil {
		log.WithError(err).Warn("could not retrieve products")
		errMsg += "상품 목록을 불러오지 못했습니다. "
		products = []*pb.Product{}
	}

	// cart: 실패해도 fatal 아님, cart_size 0으로
	cart, err := fe.getCart(r.Context(), sessionID(r))
	if err != nil {
		log.WithError(err).Warn("could not retrieve cart")
		errMsg += "장바구니 정보를 불러오지 못했습니다. "
		cart = []*pb.CartItem{}
	}
	...
	for i, p := range products {
		...
		if err != nil {
			log.WithError(err).Warnf("failed to do currency conversion for product %s", p.GetId())
			// 환율 실패 시 USD로 fallback
			price = p.GetPriceUsd()
			if !strings.Contains(errMsg, "USD") {
				errMsg += "일부 상품 가격이 기본 통화(USD)로 표시됩니다. "
			}
		}
...
func (fe *frontendServer) assistantHandler(w http.ResponseWriter, r *http.Request) {
	currencies, err := fe.getCurrencies(r.Context())
	if err != nil {
		renderHTTPError(r.Context().Value(ctxKeyLog{}).(logrus.FieldLogger), r, w, errors.Wrap(err, "could not retrieve currencies"), http.StatusInternalServerError)
		return
	}
